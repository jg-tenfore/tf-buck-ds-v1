...
import type { CommandMenuGroup } from "@/components/application/command-menu/command-menu";
import type { NavItemType } from "./config";

/** The current Tenfore Golf back-office navigation — 18 top-level sections, flattened to two tiers. */
export const TENFORE_NAV_ITEMS: NavItemType[] = [
    {
    ...
    { label: "Orders", href: "/orders", icon: ShoppingCart01, items: [{ label: "All Orders", href: "/orders/all", icon: Clipboard }] },
    {
        label: "Reports",
        href: "/reports",
    ...
    { label: "Inventory", href: "/inventory", icon: Package, items: [{ label: "Inventory Counts", href: "/inventory/counts", icon: Box }] },
    { label: "Events", href: "/events", icon: Calendar, items: [{ label: "Events", href: "/events/list", icon: Calendar }] },
    {
        label: "Bays (beta)",
        href: "/bays-beta",
        icon: Building07,
    ...
    { label: "Marketing", href: "/marketing", icon: Mail05, items: [{ label: "Email", href: "/marketing/email", icon: Mail01 }] },
    {
        label: "Admin",
        href: "/admin",
        icon: UserSquare,
...
/** Command-menu groups derived from the nav: one group per section, sub-items as searchable rows. */
export const TENFORE_COMMAND_GROUPS: CommandMenuGroup[] = TENFORE_NAV_ITEMS.map((section) => ({
    label: section.label,
    items: (section.items ?? []).map((item) => ({ label: item.label, href: item.href, icon: item.icon, hint: section.label })),
